...
import {
  ...
  downloadCanvas, generateImage, lonlatToPixel, metresPerWorldUnit, pixelToLonlat, scanCaptures, validateBbox,
} from "./wayback.js";
...
function resolveArea() {
  if (form.elements.area_mode.value === "bbox") {
    ...
    const angle = num("bbox_angle", false) ?? 0;
    if (Math.abs(angle) < 0.05) return { bbox, points: null, corridor: null, geom: null };
    const box = boxFromInputs();
    if (!box) throw new Error("Bounding box is incomplete.");
    const corridor = boxCorridor(box);                       // rotated box == corridor with no padding
    const geom = corridorGeometry(corridor.start, corridor.end, corridor.halfWidthM, corridor.padM);
    return { bbox: geom.envelope, points: null, corridor, geom };
  }
...
form.addEventListener("submit", async ev => {
  ...
  try {
    ...
    current.rotation = form.elements.area_mode.value === "bbox" && params.corridor
      ? (num("bbox_angle", false) ?? 0) : null;
...
function renderResult({ canvas, filename, meta, rotation = null }) {
  ...
  $("result-aligned").hidden = !meta.aligned || rotated;
  $("result-rotated").hidden = !rotated;
  if (meta.aligned) $("result-bearing").textContent = Math.round(meta.bearing);
  if (rotated) {
    $("result-rotation").textContent = rotation.toFixed(1);
