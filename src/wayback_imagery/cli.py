...
def build_parser() -> argparse.ArgumentParser:
    ...
    p.add_argument("--aligned", action="store_true",
                   help="With --segment: rotate so the road runs left to right and crop to a narrow corridor.")
    p.add_argument("--half-width-m", type=float, default=20.0, help="Corridor half-width in metres (with --aligned).")
    p.add_argument("--pad-m", type=float, default=30.0, help="Corridor padding past each endpoint (with --aligned).")
...
def resolve_area(args):
    ...
    if args.segment:
        ...
        return bbox_from_segment(start, end, margin_m=args.margin_m, margin_frac=args.margin_frac), points, None
    if args.aligned:
